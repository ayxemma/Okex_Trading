...
class FutureAPI(Client):
    ...
    def market_close(self, instrument_id, direction):
    ...
    # take algo orders
    def take_trigger_order(self, client_oid, instrument_id, otype, trigger_price, size, algo_price):
        params = {'client_oid': client_oid, 'instrument_id': instrument_id, 'type': otype, 'trigger_price': trigger_price,
                  'size': size, 'algo_price': algo_price}
        return self._request_with_params(POST, FUTURE_ALGOORDER, params)

    def cancel_trigger_order(self):
        pass

    def get_trigger_order(self, instrument_id):
        """
        get pending trigger orders
        """
        return self._request_without_params(GET, FUTURE_ALGOORDER + str(instrument_id) + '?order_type=1&status=1')
